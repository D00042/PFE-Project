...
if (!document.getElementById("liq-print-style")) {
  const s = document.createElement("style"); s.id = "liq-print-style";
s.textContent = `@media print { body *{visibility:hidden} #liq-print,#liq-print *{visibility:visible} #liq-print{position:absolute;left:0;top:0;width:100%} button{display:none!important} .no-print{display:none!important} }`;  
...
export default function LiquidityDashboard() {
  ...
  const kpiCards = [
    { key: "cashRatio",    label: "Cash Ratio",            good: (v) => v >= 0.2, format: (v) => v.toFixed(2),
      icon: <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke={C.blue} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg> },
    { key: "freeCashFlow", label: "Free Cash Flow",        good: (v) => v >= 0,   format: (v) => fmt(v),
      icon: <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke={C.blue} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg> },
    { key: "closingCash",  label: "Closing Cash Balance",  good: (v) => v >= 0,   format: (v) => fmt(v),
      icon: <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke={C.blue} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg> },
    { key: "openingCash",  label: "Opening Cash Balance",  good: (v) => v >= 0,   format: (v) => fmt(v),
      icon: <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke={C.blue} strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v4l3 3"/></svg> },
  ];

  return (
    <div style={S.page} id="liq-print">
      ...
      <div style={{ padding: "24px 28px 8px" }}>
        <div style={S.chartCard}>
          ...
          <div style={S.legendRow}>
            <span style={{ ...S.dot(C.positive), borderRadius: 2 }} /><span style={S.legendText}>Positive</span>
            <span style={{ ...S.dot(C.negative), borderRadius: 2 }} /><span style={S.legendText}>Negative</span>
          </div>
          {waterfall.length === 0 ? (
